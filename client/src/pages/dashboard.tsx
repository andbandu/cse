...
export default function Dashboard() {
  ...
  const filteredData = (dividendData as DividendRecord[] || []).filter(record => {
    // Search filter
    const matchesSearch = (
    ...
    // Sector filter
    const matchesSector = filter.sector === 'All Sectors' || record.sector === filter.sector;

    // Payout month filter
    const matchesMonth = filter.payoutMonth === 'All Months' || record.fyEnding === filter.payoutMonth;
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            <h1 className="text-2xl font-bold">Dividend Data Portal</h1>
